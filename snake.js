...
function isGameOver() {
	...
	if (gameOver) {
		...
		ctx.fillText("Game Over!", canvas.width / 6.5, canvas.height / 2.3);
...
function clean() {
	canvas.getElementById("game ", (value = clean));
}
...
function drawScoreEnd() {
	...
	ctx.fillText("Score : " + score, canvas.width / 3, canvas.height / 1.9);
}
function drawPlayAgain() {
	ctx.fillStyle = "white";
	ctx.font = "20px Verdana";
	ctx.fillText("Press Enter to play", canvas.width / 3.6, canvas.height / 1.65);
}
